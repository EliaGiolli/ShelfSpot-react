...
function BorrowingCTA() {
  ...
  // Theme-based classes
  const sectionBg = theme === 'light' ? 'bg-amber-50' : 'bg-amber-950';
  const headingText = theme === 'light' ? 'text-amber-700' : 'text-amber-300';
  const paragraph = theme === 'light' ? 'text-gray-900' : 'text-white';
  ...
  return (
    <>
      <section className={`py-16 ${sectionBg} px-6 max-w-6xl mx-auto text-center`}>
